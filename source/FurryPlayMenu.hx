...
function mouseHover(elapsed:Float) {
    ...
    if (DAWtween != null) DAWtween.cancel();
    if (HWtween != null) HWtween.cancel();
    if (NDWtween != null) NDWtween.cancel();

    if (curSelect > 0) curSelect = 0;

    if (mouseX > DAW.x && mouseX < DAW.x + DAW.width &&
        mouseY > DAW.y && mouseY < DAW.y + DAW.height) {
    ...
    if (mouseX > HW.x && mouseX < HW.x + HW.width &&
        mouseY > HW.y && mouseY < HW.y + HW.height) {
    ...
    if (mouseX > NDW.x && mouseX < NDW.x + NDW.width &&
        mouseY > NDW.y && mouseY < NDW.y + NDW.height) {
