...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var songsInfo:Map<String, Array<String>> = [
			...
			'protogen' => ['', 'COVER SONG'],
			'kaiju-paradise' => ['', 'CUSTOM SONG'],

			'programming-an-ammar' => ['ammar', 'COVER SONG'],
			'identity-crisis' => ['ammar', 'COVER SONG'],
			'note-nova' => ['ammar', 'CUSTOM SONG'],
			'twisted-truths' => ['ammar', 'CUSTOM SONG']
		];
		var diffColor:Map<String, FlxColor> = ['hard' => 0xFFFF0000, 'easy' => 0xFF0077FF, 'insane' => 0xFF8C00FF];
		var songName:String = Paths.formatToSongPath(SONG.song);
		...
		if (desc == null)
			desc = '';
		if (diffColor[ClientPrefs.aDifficulty.toLowerCase()] != null)
			color = diffColor[ClientPrefs.aDifficulty.toLowerCase()];
		...
		{
			if (Paths.formatToSongPath(PlayState.SONG.song.toLowerCase()) != "google"
				&& Paths.formatToSongPath(PlayState.SONG.song.toLowerCase()) != "shut-up-aac"
				&& Paths.formatToSongPath(PlayState.SONG.song.toLowerCase()) != "kaiju-paradise"
				&& Paths.formatToSongPath(PlayState.SONG.song.toLowerCase()) != "twitter-argument"
				&& Paths.formatToSongPath(PlayState.SONG.song.toLowerCase()) != "hate-comment"
				&& Paths.formatToSongPath(PlayState.SONG.song.toLowerCase()) != "furry-appeared")
			{
	...
	function createHUD():Void
	{
		...
		comboSpr.alpha = 0;
		if (ClientPrefs.downScroll && !ClientPrefs.middleScroll)
			comboSprY = 720 - (comboSpr.frameHeight) + 36;
	...
	public function startCountdown():Void
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			FlxTween.tween(rateTxt, {x: rateTxt.x + (HUDtoRight ? -400 : 400)}, Conductor.crochet / 1000 * 2,
				{ease: FlxEase.quadOut, startDelay: Conductor.crochet / 1000});
				...
				switch (swagCounter)
				{
					...
					case 2:
						if (curStage.toLowerCase() == 'discordstage')
						{
							...
							if (countdownTweenColor != null)
								countdownTweenColor.cancel();
					...
					case 3:
						if (curStage.toLowerCase() == 'discordstage')
						{
							...
							countdownText.screenCenter();
							countdownTweenColor = FlxTween.color(countdownText, Conductor.crochet / 1000, 0xFFBCC6FF, 0x006078FF, {
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
		{
			...
			if (true)
			{
				if (hardVocals)
				{
					if (!Paths.voicesExist(PlayState.SONG.song, '-opponent-hard')
						|| !Paths.voicesExist(PlayState.SONG.song, '-player-hard'))
					{
						trace('no hard saperate voices song found ');
						hardVocals = false;
					}
					else if (Paths.voicesExist(PlayState.SONG.song, '-opponent-hard')
						&& Paths.voicesExist(PlayState.SONG.song, '-player-hard'))
					{
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			if (comboSprTween != null)
				comboSprTween.active = false;
	...
	override public function update(elapsed:Float)
	{
		...
		{
			iconP1.x = healthBar.x
				+ (healthBar.width * (1 - (lerpHealth / 2)))
				+ (150 * iconP1.scale.x - 150) / 2
				- iconOffset
				+ 5
				+ iconP1.sprOffsetX;
		...
		switch (eventName)
		{
			...
			case 'Add Camera Zoom':
				if (ClientPrefs.camZooms && FlxG.camera.zoom < 1.35)
				{
					...
					if (Math.isNaN(frontZoom))
						frontZoom = 0.02;
					if (Math.isNaN(backZoom))
						backZoom = 0.04;
			...
			case 'Remove Message':
				...
				if (val == 1)
					if (discordOpponent != null)
						discordOpponent.deleteMessage();
					else if (discordPlayer != null)
						discordPlayer.deleteMessage();
		}
	...
	public function endSong():Void // ! EXTREME BUG : SOMETIME SONG END DOESN'T RUN endSong()
	{
		...
		var isCheat:Bool = haveUsePractice
			|| haveUseBotplay
			|| healthGain > 1
			|| healthLoss < 1
			|| !ClientPrefs.getGameplaySetting('mechanics', true)
			|| !ClientPrefs.getGameplaySetting('specialnotes', true)
			|| !ClientPrefs.getGameplaySetting('modchart', true);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (!cpuControlled)
		{
			...
			if (comboSprTween != null)
				comboSprTween.cancel();
			comboSpr.y = comboSprY;
			comboSprTween = FlxTween.tween(comboSpr, {alpha: 0, y: comboSprY - (ClientPrefs.downScroll ? -10 : 10)}, 0.2, {ease: FlxEase.quadIn,
				startDelay: 0.2});
		}
	}
	...
	private function onKeyPress(event:KeyboardEvent = null):Void
	{
		...
		if (!cpuControlled
			...
			&& !paused
			&& key > -1
			&& (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode || mobileMode))
		{
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			{
				if (lightHUDTween != null)
					lightHUDTween.cancel();
				lightHUD.color = noteColor[note.noteData % 4];
				lightHUD.alpha = 0.75;
				lightHUDTween = FlxTween.tween(lightHUD, {alpha: 0}, 1, {onComplete: function(tw:FlxTween)
				{
					lightHUDTween = null;
				}});
			}
