...
class MainMenuStateAmmar extends MusicBeatState
{
    ...
    var menuList:Array<Array<String>> = [
        ...
        ['Credits', 'See the cool people who made this mod! :3'],
    ...
    var optionsList:Array<Array<String>> = [
        ...
        ['Reduce Shakiness', 'Reduce the screen shake Intensity. Useful for People who easily get motion sick', 'reduceShakiness'],
        ['Silly Bounce', 'Make The whole menu bop to the music', 'sillybop'],
    ...
    var songsDifficulty:Map<String, Array<Int>> = [
        ...
        'Protogen'           => [8, 9, 10, 11],
        'Kaiju Paradise'     => [6, 8, 11],  // back for now || not editing kaiju rn since its being removed for release
    ...
    **/
    ...
    var creditsList:Array<Array<Dynamic>> = [ //Name, Credit Icon, Description, Link, Quotes
        ...
        ["uwenalil",        "dave",     "Programmer, Modcharter, Visual Artist, Charter, Tester",                   "https://uwenalil.netlify.app/",                            '"EVERYONE, SHUT UP"'],
        ...
        for (menu in menuList)
            {
                ...
                var redexcute =  ~/r/g;
                var menuItem:MenuText = new MenuText(-800, 80 + (idd*130), 0, ClientPrefs.cute ? redexcute.replace(menuName, 'w') : menuName, 100);
    ...
    function createCredits():Void 
    {
        curPage = "credits";
        if (bg.color != menuColor["credits"]) 
            {
                if (bgColorTween != null && bgColorTween.active)
                    bgColorTween.cancel();
                bgColorTween = FlxTween.color(bg, 1, bg.color, menuColor["credits"]);
            }

        var idd:Int = 0;
        for (creditsGroup in creditsList)
            {
                var creditsName:String = creditsGroup[0];
                ...
                var menuIcon:MenuSprite = new MenuSprite(1300, 0, Paths.image("credits/"+creditsImage));
                menuIcon.partner = menuItem;
                menuIcon.objectID = idd;
                menuIcon.scale.set(0.75, 0.75);
                menuIcon.updateHitbox();
                menuIcon.followPartner = true;
                menuIcon.addX = menuItem.width + 20;
                menuIcon.addY = -(menuIcon.height / 4);
                menuIcon.partOf = "credits";
                menuIcons.add(menuIcon);
                menuIcon.cameras = [camHUD];
    ...
    override function update(elapsed:Float) 
    {
        ...
        var intensityPercent:Float = curSelected/(menuItems.length-1);
        ...
        hudFollow.y = FlxMath.lerp(hudFollow.y, (720/2) + (curPage == 'credits' ? intensityPercent * 69 : intensity), elapsed*8);
        if (ClientPrefs.sillybop) {
            bgFollow.y = (720/2) + Math.abs(Math.sin( ((curDecBeat/2)%1) * 2 * Math.PI)) * 6;
        } else {
            bgFollow.y = FlxMath.lerp(bgFollow.y, (720/2) + (intensityPercent*20)/*(intensity/2)*/, elapsed*8);
        }
    ...
    function selectItem():Void {
        ...
        if (curPage != 'freeplay' && curPage != 'story menu' && curPage != 'badges' && curPage != 'credits') {
            selectedSomething = true;
            itemMove();
        }
        //curSelected = 0;
        ...
        if (curPage == "freeplay") {
            ...
            switch (selectedButton) {
                ...
                case 'credits':
                    createCredits();
                    #if desktop
                        DiscordClient.changePresence("In the "+(ClientPrefs.cute ? 'Cute ' : '')+"Credits", null);
                    #end
                case 'options':
                    createOptions(0);
                    #if desktop
            ...
            trace('badge');
        } else if (curPage == "credits") {
            trace('linklol');
            if ((creditsList[curSelected][3] == null || creditsList[curSelected][3].length > 4))
            {
                CoolUtil.browserLoad(creditsList[curSelected][3]);
            }
    ...
    private function displayList():Void {
        ...
        menuItems.forEachAlive(function(member:MenuText){
            ...
            if (!member.noMove) {
                ...
                if (member.partOf == "freeplay" || member.partOf == "badges" || member.partOf == "credits") {
                    if (getSwipeEnable()) {
                        //insert swiping here
    ...
    private var dragEnable:Bool = #if MOBILE true #else true #end; //MOBILE
