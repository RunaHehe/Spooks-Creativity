...
local dialogue = {
    ...
    {848, 'a', 'stop it'},
    {858, 'a', ''},
    {864, 'a', 'leave me alone'},
    {876, 'a', ''},

    {1040, 'f', 'I\'m you from the past'},
    {1064, 'f', ''},

    {1480, 'f', 'You\'re cute :3'},
    {1492, 'a', '...'},

    {1546, 'a', 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'}
}
...
function onCreatePost()
    ...
    makeGraphic('white', 1280, 720, 'DC000F')
    ...
    makeLuaText('furryText', 'uwu', 700, getProperty('dad.x') + 50, - 15)
    ...
    makeLuaText('ammarText', '', -5000, getProperty('boyfriend.x') + (getProperty('boyfriend.width')/2), 15)
...
function createLight(tag, x, y, scroll, scale)
    ...
    addLuaSprite(tag)
    setProperty(tag..'.color', getColorFromHex("DC000F"))
end
        ...
        if curStep % 8 == 0 then
            for i = 0, 3 do
                if (i % 2 == 0 and curStep % 16 == 0) or (i % 2 == 1 and curStep % 16 == 8) then
                    setProperty('glow'..i..'.color', getColorFromHex('DC000F'))
                else
