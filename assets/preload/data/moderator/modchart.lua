...
function easeSetup()
    ...
    for beat = 0, (4*8-1) do
        local time = 64 + beat
        local turn = beat >= 16 and "op" or "bf"
    ...
    ease(94, 1, "linear", "1, opHid"); ease(94, 2, "linear", "1, opWiggle")
    ...
    for beat = 0, (4*8)-3 do
        ...
        if beat%4 == 0 then
            ease(time, 1 ,'quadOut', '20, protate:y')
            ease(time, 1 ,'quadOut', '-20, orotate:y')
        elseif beat%4 == 1 then
            ease(time, 1 ,'quadOut', '0, protate:y')
            ease(time, 1 ,'quadOut', '0, orotate:y')
